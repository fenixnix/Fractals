...
namespace Fractals {
    public class FractalLine : MonoBehaviour {
        ...
        public void Test_SubDivid() {
            ...
            SubDivid(n,points,new Line(Vector3.zero, Vector3.up), Random.Range(Amplify / 2, Amplify));
            pointList.Add(points);
        }

        public void SubDivid(int n,List<Vector3> points, Line line, float w = .2f) {
            if(n == 0) {
                points.Add(line.b);
                return;
            }
            var next = n - 1;
            var W = w;
            var tmpPoint = line.MidPoint + new Vector3(Random.Range(-W, W),
                (Random.Range(-W, W)), (Random.Range(-W, W)));
            SubDivid(next, points, new Line(line.a, tmpPoint), W / 2);
            SubDivid(next, points, new Line(tmpPoint, line.b), W / 2);
        }
